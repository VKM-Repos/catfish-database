...
interface CreatePostMutationHookArgs<RequestSchema extends z.ZodType, ResponseSchema extends z.ZodType> {
  ...
  mutationOptions?: Omit<UseMutationOptions<z.infer<ResponseSchema>, Error, z.infer<RequestSchema>>, 'mutationFn'>
  /** Callback function called on successful mutation */
  onSuccess?: (data: z.infer<ResponseSchema>, variables: z.infer<RequestSchema>, queryClient: QueryClient) => void
  /** Callback function called on mutation error */
  onError?: (error: Error, variables: z.infer<RequestSchema>, queryClient: QueryClient) => void
...
export function createPostMutationHook<RequestSchema extends z.ZodType, ResponseSchema extends z.ZodType>({
  endpoint,
  requestSchema,
  responseSchema,
  mutationOptions,
  ...
  return () => {
    ...
    return useMutation({
      ...
      onError: (error, variables) => onError?.(error as Error, variables, queryClient),
      onSettled: (data, error, variables) => onSettled?.(data, error as Error | null, variables, queryClient),
      ...mutationOptions,
    }) as UseMutationResult<z.infer<ResponseSchema>, Error, z.infer<RequestSchema>>
  }
}
