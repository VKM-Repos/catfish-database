...
export default function FeedingReportsDetailsModal() {
  ...
  const samplingReport = [
    ...
    { label: 'Quantity', value: samplingData?.quantity ?? '-' },
    { label: 'Total Weight Harvested', value: samplingData?.totalWeightHarvested ?? '-' },
    { label: 'Cost per KG', value: samplingData?.costPerKg ?? '-' },
  ]
  if (!id) return null
          ...
          <FlexBox direction="col" gap="gap-6" className="w-full py-4">
            ...
            <div className="mt-6 flex w-full justify-between space-x-2">
              <Button
                className="w-full"
                variant="outline"
                onClick={() => navigate(`${paths.dashboard.reports.root}?tab=harvest-reports`)}
              >
                Cancel
              </Button>
            </div>
