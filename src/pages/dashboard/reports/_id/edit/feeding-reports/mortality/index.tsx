...
export function UpdateMortality({ handleGoBack }: { handleGoBack: () => void }) {
  ...
  const useUpdateMortality = createPutMutationHook({
    endpoint: `/mortalities/${location.state.report.id}`,
    requestSchema: z.any(),
  ...
  const onSubmit = async (data: z.infer<typeof mortalitySchema>) => {
    try {
      const updateMortalityData = {
        mortalityNumber: data.mortalityNumber,
        frequency: 'DAILY',
        time: combinedDateTime,
        date,
      }
      await updateMortality.mutateAsync(updateMortalityData)
      queryClient.refetchQueries(['fish-mortality'])
      setOpenDialog(true)
    } catch (err) {
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="w-full space-y-6">
          ...
          <div className="flex justify-between bg-neutral-100 p-5">
            ...
            <Button disabled={updateMortality.isLoading} type="submit">
