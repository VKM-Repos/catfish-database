...
export function DateTimePicker({
  ...
  const { selectedDate, setSelectedDate, setCombineDateTime, selectedTime, setSelectedTime, combineDateTime } =
    useDateStore()
  ...
  const handleDateTimeChange = (date: string, time: string | undefined) => {
    if (date && time) {
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    handleDateTimeChange(dateValue, selectedTime)
    ...
    <FlexBox direction="col" className="w-full lg:w-fit">
      <FlexBox align="center" gap={'lg:gap-5 gap-[10px]'} className="w-full justify-between lg:w-fit">
        ...
        <div className="flex w-full max-w-[50%] flex-col lg:max-w-full ">
          ...
          <div
            className={`focus-within:ring-offset-background flex max-h-fit items-center rounded-md border ${
              activeInputs.date ? 'bg-primary-100' : ''
        ...
        <div className="flex w-fit flex-col lg:w-full">
          ...
          <div
            className={`focus-within:ring-offset-background flex max-h-fit items-center rounded-md border ${
              activeInputs.time ? 'bg-primary-100' : ''
            } border-neutral-200 focus-within:ring-2 focus-within:ring-primary-500 focus-within:ring-offset-2`}
          >
            <div className="w-full">
              <Input
            ...
            <div
              className={`h-10 cursor-pointer rounded-br-md rounded-tr-md px-3 py-[.65rem] text-xs ${
                selectedTime ? 'bg-primary-500 text-white' : 'bg-neutral-100 text-neutral-400'
              }`}
      ...
      {!combineDateTime && <Text className=" text-sm text-error-500">Select date time to continue</Text>}
    </FlexBox>
  )
}
