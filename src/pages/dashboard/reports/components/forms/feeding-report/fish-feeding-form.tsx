...
export function DailyFeeding({ handleNext }: { handleNext?: () => void }) {
  ...
  const { formData, activeInputs, setFormData, setActiveInput, reset: resetStore } = useDailyFeedingStore()
  ...
  const onSubmit = async (data: z.infer<typeof dailyFeedingSchema>) => {
    try {
      ...
      const feedingData = {
        ...
        feedingTime: new Date(`${new Date().toISOString().split('T')[0]}T${data.feedTime}`).toISOString(),
      }
    ...
    <>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="w-full space-y-6">
          <div>
            <FlexBox direction="row" justify="between" align="center" className="mb-10 border-0 border-b">
              <div className="p-5">
          ...
          <div className="flex justify-between bg-neutral-100 p-5">
            ...
            <Button disabled={createDailyFeeding.isLoading} type="submit">
              Continue
            </Button>
          </div>
        </form>
