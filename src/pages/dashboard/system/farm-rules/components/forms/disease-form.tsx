...
export function DiseaseForm({ mode, initialValues, onSuccess, onClose }: FarmerProps) {
    ...
    <>
      <div className="absolute inset-x-0 top-0 w-full border-b border-b-neutral-200 py-2">
        <Heading className="text-center" level={6}>
          {mode === 'create' ? 'Add disease type' : ' Edit feed type'}
        </Heading>
      </div>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
          ...
          <Text className="flex items-center gap-2 text-sm font-medium text-neutral-700">
            Disease name <span className="font-bold text-red-500">*</span>
