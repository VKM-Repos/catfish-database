...
export const pondSchema = z.object({
  ...
  length: z.string().regex(/^\d+(\.\d+)?$/, 'Pond length must be a valid number'),
  breadth: z.string().regex(/^\d+(\.\d+)?$/, 'Pond breadth must be a valid number'),
  height: z.string().regex(/^\d+(\.\d+)?$/, 'Pond height must be a valid number'),
